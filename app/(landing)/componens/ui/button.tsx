...
const Button = ({children, className, variant = "primary", size = "normal", ...props}: TButtonProps) => {
    const  baseStyles= "inline-flex gap-2 duration-300 justify-center items-center cursor-pointer hover:scale-106";
    ...
    return (
        <button className={`${baseStyles} ${varians[variant]} ${sizes[size]} ${className}`} {...props}> {children} </button>
    )
}
